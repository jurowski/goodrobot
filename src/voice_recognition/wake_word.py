...
class WakeWordDetector:
    ...
    def __init__(
        ...
        logger.debug(
            f"Wake word: {self.wake_word}, Sensitivity: {self.sensitivity}, Sample rate: {self.sample_rate}"
        )
        ...
        # Initialize and raise exception if it fails
        if not self.initialize():
            raise RuntimeError(
                "Failed to initialize wake word detector. Check your Picovoice access key and settings."
            )
    ...
    def _process_calibration(self):
        ...
        try:
            ...
            logger.info(
                f"Calibration complete. New sensitivity: {new_sensitivity}, New noise threshold: {new_noise_threshold}"
            )
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            # Check if wake word is a built-in keyword
            built_in_keywords = pvporcupine.KEYWORDS
            if self.wake_word not in built_in_keywords:
                logger.warning(
                    f"Wake word '{self.wake_word}' not in built-in keywords, defaulting to 'jarvis'"
                )
                self.wake_word = "jarvis"

            self.porcupine = pvporcupine.create(
                access_key=self.access_key,
                keywords=[self.wake_word],  # Use keywords for built-in wake words
                sensitivities=[self.sensitivity],
            )

            logger.info(
                f"Successfully initialized wake word detector with wake word '{self.wake_word}'"
            )
            return True
        ...
        try:
            ...
            with io.BytesIO(audio_data) as audio_buffer:
                try:
                    ...
                    logger.debug(
                        f"Audio format: shape={audio_array.shape}, sample_rate={sample_rate}, dtype={audio_array.dtype}"
                    )
                    ...
                    if sample_rate != self.sample_rate:
                        ...
                        samples = int(
                            (len(audio_array) * self.sample_rate) / sample_rate
                        )
                    ...
                    if energy < self.noise_threshold:
                        logger.debug(
                            f"Audio energy {energy} below noise threshold {self.noise_threshold}"
                        )
                        return False
                    ...
                    logger.warning(
                        f"Failed to read with soundfile: {e}, trying direct conversion"
                    )
            ...
            if len(audio_array) < frame_length:
                logger.debug(
                    f"Padding audio from {len(audio_array)} to {frame_length} samples"
                )
    ...
    def cleanup(self):
        ...
        try:
            ...
            self._is_listening = False
            logger.info("Cleaned up wake word detector resources")
        except Exception as e:
