...
def test_websocket_connection():
    ...
    with client.websocket_connect("/ws/test_client") as websocket:
        # Test config message
        config = {"type": "config", "sensitivity": 0.5, "noiseThreshold": 0.15}
...
try:
    ...
    def test_voice_interface_ui(browser_page):
        ...
        try:
            ...
            print(
                f"Failed to wait for Listening status. Current status: {current_status}"
            )
            print("\nConsole messages:")
        ...
        try:
            ...
            print(
                f"Failed to wait for Stopped status. Current status: {current_status}"
            )
            print("\nConsole messages:")
            for msg in console_messages:
                print(f"  {msg}")
